...
library(fpp3)
library(here)
here::i_am("R/w2d1-solutions.qmd")
```
...
The observations are quarterly. An upward trend is apparent until 1980, after which the number of clay bricks being produced starts to decline. A seasonal pattern is evident in this data. Some sharp drops in some quarters can also be seen.
...
Observations are made once per year. Canadian lynx trappings are cyclic, as the extent of peak trappings is unpredictable, and the spacing between the peaks is irregular but approximately 10 years.
...
Interval is daily. Looking closer at the data, we can see that the index is a Date variable. It also appears that observations occur only on trading days, creating lots of implicit missing values. Stock prices for these technology stocks have risen for most of the series, until mid-late 2018.
...
Data is available at 30 minute intervals. Appears to have an annual seasonal pattern, where demand is higher during summer and winter. Can't see much detail, so let's zoom in.
...
```{r}
dl_loc <- here('raw-data/tute1.csv')
download.file("http://OTexts.com/fpp3/extrafiles/tute1.csv",
              destfile = dl_loc)

tute1 <- readr::read_csv(dl_loc)
...
```{r}
tourism_dl_loc <- here('raw-data/tourism.xlsx')
download.file("http://OTexts.com/fpp3/extrafiles/tourism.xlsx",
              destfile = tourism_dl_loc)


my_tourism <- readxl::read_excel(tourism_dl_loc) |>
  mutate(Quarter = yearquarter(Quarter)) |>
  as_tsibble(
    index = Quarter,
